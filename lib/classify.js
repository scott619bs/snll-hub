...
export function parseCSV(text) {
  ...
  // GC full exports have section header rows before the real column header row.
  // Find the first row that looks like actual column headers (contains "Number" or "Last" or "First")
  // Search ALL lines, not just first 10, in case of extra blank/section rows
  let headerIdx = -1
  for (let i = 0; i < lines.length; i++) {
    const cols = splitCSVLine(lines[i]).map(h => h.toLowerCase().trim())
    if (cols.includes('number') || cols.includes('last') || cols.includes('first') || cols.includes('player') || cols.includes('name')) {
      headerIdx = i
      break
    }
  }

  // Fallback: if no standard header row found, use first line
...
export function classifyCSV(headers) {
  ...
  // GC full export: has batting + pitching + fielding columns all in one header row
  const hasBatting  = h.includes('avg') || h.includes('obp') || h.includes('rbi')
  const hasPitching = h.includes('era') || h.includes('ip') || h.includes('bf')
  const hasFielding = h.includes('tc')  || h.includes('fpct') || h.includes('po')
  const hasBasic    = h.includes('number') || h.includes('last') || h.includes('first')
  if (hasBatting && hasPitching && hasFielding) return { type: 'full', confidence: 'high', reason: 'Contains batting, pitching, and fielding columns — full season export' }
  if (hasBasic && hasBatting && h.includes('ops') && h.includes('slg')) return { type: 'full', confidence: 'high', reason: 'Full GC export format detected (Number/Last/First + AVG/OBP/OPS/SLG)' }
...
Use null for missing values. Skip Totals rows. team_name should be "Myers" for our team.`
  ...
  const MIME_MAP = {
    ...
    'image/webp': 'image/webp','image/heic': 'image/jpeg',
    'image/heif': 'image/jpeg',
  }
  const safeMime = MIME_MAP[(input.mimeType || '').toLowerCase()] || 'image/jpeg'

  const content = inputType === 'image'
    ? [
        { type: 'image', source: { type: 'base64', media_type: safeMime, data: input.data } },
